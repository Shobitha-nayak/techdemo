...
export const getServerSideProps = async () => {
  ...
  for (const ticker of tickers) {
    try {
      const response = await axios.get(`http://192.168.58.2:30001/api/kpi/${ticker}`);
      reports[ticker] = response.data;
    } catch (error) {
      reports[ticker] = { error: 'Failed to fetch data' };
    }
  ...
  const topGainersLosersResponse = await axios.get(`http://192.168.58.2:30001/api/top-gainers-losers`);
  const topGainersLosers = topGainersLosersResponse.data;
  return {
    props: {
      reports,
